...
const AREA_BLUEPRINTS: Omit<AreaSeed, "type">[] = [
...
const PRODUCTS: Omit<ProductSeed, "type">[] = [
...
function pickSome<T>(random: () => number, items: readonly T[], count: number): T[] {
    ...
    for (let i = 0; i < count && clone.length > 0; i += 1) {
        ...
        selected.push(clone[idx] as T);
        clone.splice(idx, 1);
    }
    return selected;
}

function toUtc7Timestamp(value: Date): string {
...
async function ensureSeedShopId(): Promise<number> {
    const existing = await AppDataSource.query(
        `SELECT "ID" FROM "Shop" WHERE "Email" = $1 LIMIT 1`,
        [SEED_SHOP_EMAIL],
    ) as SqlIdRow[];
...
async function cleanShopData(shopId: number): Promise<void> {
...
async function seedRealisticData(): Promise<void> {
    ...
    try {
        ...
        const areasToCreate: AreaSeed[] = AREA_BLUEPRINTS.map((area, index) => ({
            ...area,
            type: index <= 3 ? areaTypeTable : areaTypeRoom,
        }));

        const productsToCreate: ProductSeed[] = PRODUCTS.map((product, index) => ({
            ...product,
        ...
        const membershipRows = await AppDataSource.query(
            ...
            `,
            [shopId],
        ) as Array<{ ID: number; Name: string; MinSpent: string; DiscountPercent: string }>;

        const memberships: MembershipRow[] = membershipRows.map((row) => ({
            id: Number(row.ID),
        ...
        const members: MemberRow[] = memberRows.map((row) => ({ id: Number(row.ID), name: row.Name }));
        ...
        const areaRows = await AppDataSource.query(
            ...
            `,
            [shopId],
        ) as Array<{ ID: number; Name: string; HourlyPrice: string; MaxCapacity: number }>;

        const areas: AreaRow[] = areaRows.map((row) => ({
            id: Number(row.ID),
            name: row.Name,
        ...
        for (const categoryName of categoryNames) {
            await AppDataSource.query(
                `INSERT INTO "GameCategory" ("ShopID", "Name") VALUES ($1, $2)`,
                [shopId, categoryName],
            );
        }
        ...
        const categoryIdByName = new Map<string, number>(
            categoryRows.map((row) => [row.Name, Number(row.ID)]),
        );
        ...
        const productRows = await AppDataSource.query(
            ...
            `,
            [shopId],
        ) as Array<{ ID: number; Name: string; Price: string }>;
        ...
        const spentByMemberId = new Map<number, number>();
...
seedRealisticData().catch((error: unknown) => {
