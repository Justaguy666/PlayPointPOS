...
public class ConfigurationServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task SaveAndLoadAsync_RetainsServerSettingsRegardlessOfRememberMe()
    {
        string configPath = Path.Combine(_tempDirectory, "config.json");

        var service = new ConfigurationService(configPath);
        await service.SaveAsync("https://server", 3000, rememberMe: true);
        ...
        await service.SaveAsync("https://server", 8080, rememberMe: false);
        ...
        Assert.True(notRemembered.IsConfigured);
    }
    ...
    public async Task SaveAndLoadAsync_RetainsLocalizationPreferences()
    {
        ...
        Assert.Equal(4000, reloaded.Port);
    ...
    public async Task LoadAsync_UsesDefaultPortWhenPortIsMissing()
    {
        ...
        await File.WriteAllTextAsync(
            configPath,
            "{\"serverAddress\":\"https://server\",\"rememberMe\":true}");

        var service = new ConfigurationService(configPath);
        await service.LoadAsync();

        Assert.Equal("https://server", service.ServerAddress);
        Assert.Equal(4000, service.Port);
